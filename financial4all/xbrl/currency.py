# financial4all/xbrl/currency.py
"""
...
class CurrencyConverter:
    ...
    def _detect_home_currency(self) -> str:
        ...
        """
        try:
            facts_df = self.xbrl.facts.query().to_dataframe()
        except Exception:
            return "USD"  # Default if facts query fails

        currency_counts: Counter = Counter()

        for _, row in facts_df.iterrows():
            unit_ref = row.get('unit_ref')
            if unit_ref and hasattr(self.xbrl, 'units') and unit_ref in self.xbrl.units:
